...
C
     ...
     3 TABLE_K(2,20),TABLE_C(2,20),TABLE_KP(2,20),Mat_ID(9999999,8)
...
C     Conductivity of Powder
      DATA TABLE_KP/ 0.63,20.0,0.828356164,100.0,1.07630137,200.0,1.324246575,300.0,
     &1.572191781,400.0,1.820136986,500.0,2.068082192,600.0,2.316027397,700.0,
     &2.44,750.0,2.563972603,800.0,2.811917808,900.0,2.747465753,995.0,3.307808219,1100.0,
     &3.555753425,1200.0,3.80369863,1300.0,4.051643836,1400.0,4.299589041,1500.0,
     &4.547534247,1600.0,4.671506849,1650.0,15.0917,1655.0/
C     Specific Heat
...
C     Conductivity of Powder
      IF (statev(1) .EQ. 0) THEN
        IF (TempatTdT .LE. TABLE_KP(2,1)) THEN
          COND = TABLE_KP(1,1)
          DCOND = 0.0d0
        ELSEIF (TempatTdT .GE. TABLE_KP(2,20)) THEN
          COND = TABLE_KP(1,20)
          DCOND = 0.d0
        ELSEIF (TempatTdT .GT. TABLE_KP(2,1) .AND. TempatTdT .LT. TABLE_KP(2,20)) THEN
          DO K1 = 1,19
            TL1 = TABLE_KP(2, K1+1)
...
C     time(2) -- total time; time(1) -- step time
      t=time(2)
      pi=3.1415926
      phi = 0.55
      eta = 0.9
...
C     defining position parameters
      ...
      if (t<0.01) then
	      flux(1) = 0.0
	  else if (t .GE. 0.01d0 .AND. t .LE. 0.015d0) then
          x0 = 0.0 + v*(t-0.01)
          y0 = 0.0 
          z0 = 5.07
          Hs = 2.0*U*Ib/(pi*phi**2.0)*exp(-2.0*((x-x0)**2.0 + (y-y0)**2.0)/phi**2.0)
          Iz = 1.0/0.75*(-2.25*((z0-z)/S)**2 + 1.5*((z0-z)/S) + 0.75)
          IF (z<=5.07 .AND. z>=5.008) THEN
              Uz = 1.0
          ELSE IF (z<5.008 .AND. z>=0.0) THEN
              Uz = 0.0
          END IF
